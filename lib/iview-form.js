  ...
  methods: {
    ...
    getFormItem: function getFormItem(h, item, content) {
      ...
      if (typeof item.render === 'function') {
        ...
        return h(getPrefix('form-item', this.lib), __WEBPACK_IMPORTED_MODULE_1_babel_runtime_core_js_object_assign___default()(settings, item.settings), [this.renderTitle(h, item), content]);
      }
    },
    ...
    renderTitle: function renderTitle(h, item) {
      return h(
        ...
        [item.required === true ? h(
          ...
          { style: 'color: font' },
          ['*']
        ) : '', typeof item.renderTitle === 'function' ? h('span', [item.renderTitle(this.getHypeScript(), item, this.getFormBykey(item.key))]) : h('span', [item.title])]
      );
    },
